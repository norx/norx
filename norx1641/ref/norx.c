...
#elif   NORX_W == 64 /* NORX-64 specific */
...
#else
    #error "Invalid word size for parallel mode."
#endif
}
#endif
...
#if NORX_W == 8
    memcpy(tag, lastblock, BYTES(RATE));
    norx_inject_tag(state, FINAL_TAG);
    norx_permutation(state);
#if defined(NORX_DEBUG) && NORX_D == 1
    printf("End of tag extraction\n");
    norx_print_state(state);
#endif
    for (i = 0; i < WORDS(RATE); ++i)
        STORE(lastblock + i * BYTES(NORX_W), S[i]);
    memcpy(tag + BYTES(RATE), lastblock, BYTES(RATE));
#else /* NORX16, NORX32, NORX64 */
