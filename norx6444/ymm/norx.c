...
#define UNSLICE_MSG_V4(u, v) do {                  \
  ...
  t3 = _mm256_unpackhi_epi64(v[ 2], v[ 3]);        \
  u[ 0] = _mm256_permute2x128_si256(t0, t2, 0x20); \
  u[ 3] = _mm256_permute2x128_si256(t0, t2, 0x31); \
  u[ 6] = _mm256_permute2x128_si256(t1, t3, 0x20); \
  ...
  u[ 8] = _mm256_permute2x128_si256(t1, t3, 0x20); \
  u[11] = _mm256_permute2x128_si256(t1, t3, 0x31); \
} while(0)
*/
...
#define ENCRYPT_BLOCK_V4(S, OUT, IN) do {       \
    ...
    UNSLICE_STATE_V4(M, S);                    \
    for(k = 0; k < WORDS(NORX_R); ++k) {       \
      STOREU((OUT) + 32 * k, M[k]);            \
    }                                          \
} while(0)

#define BRANCH(S, A, B, C, D) do { \
  __m256i T[16]; \
  ...
  /* T[ 3] = T[ 7] = T[11] = T[15] = D; */ \
  /* SLICE_STATE_V4(S, T); */ \
  S[13] = XOR(S[13], _mm256_set_epi64x(3, 2, 1, 0)); \
} while(0)

#define MERGE(A, B, C, D, S) do { \
  __m256i T[16]; \
  INJECT_DOMAIN_CONSTANT_V4(S, MERGE_TAG); \
  UNSLICE_STATE_V4(T, S); \
  A = XOR(XOR(T[ 0], T[ 4]), XOR(T[ 8], T[12])); \
  B = XOR(XOR(T[ 1], T[ 5]), XOR(T[ 9], T[13])); \
  C = XOR(XOR(T[ 2], T[ 6]), XOR(T[10], T[14])); \
  D = XOR(XOR(T[ 3], T[ 7]), XOR(T[11], T[15])); \
} while(0)

#define ENCRYPT_DATA_V4(A, B, C, D, OUT, IN, INLEN)        \
do                                                         \
{                                                          \
  if (INLEN > 0) {  \
    __m256i S[16], T[16]; \
    ...
    for(j = 0; j < 4; ++j) {\
      INJECT_DOMAIN_CONSTANT(T[4*j+0], T[4*j+1], T[4*j+2], T[4*j+3], MERGE_TAG);\
      PERMUTE(T[4*j+0], T[4*j+1], T[4*j+2], T[4*j+3]);\
    }\
    A = XOR(XOR(T[ 0], T[ 4]), XOR(T[ 8], T[12]));\
    B = XOR(XOR(T[ 1], T[ 5]), XOR(T[ 9], T[13]));\
    C = XOR(XOR(T[ 2], T[ 6]), XOR(T[10], T[14]));\
